  ...
  return (
    <div className="page overflow-y-auto no-scrollbar animate-fade-in">
      ...
      <div className="relative px-5 pt-12 pb-8 overflow-hidden">
        {/* Background glow */}
        <div className="absolute -top-24 -right-24 w-64 h-64 rounded-full bg-gold-500/10 blur-3xl pointer-events-none" />
        <div className="absolute top-32 -left-16 w-48 h-48 rounded-full bg-gold-500/5 blur-2xl pointer-events-none" />
        ...
        <div className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-3">
            ...
            <span className="font-display font-bold text-lg text-gradient-gold">GoldEye</span>
          </div>
          <button
            id="change-language"
        ...
        <button
          ...
          className="btn-gold w-full text-lg py-5 mb-4 animate-pulse-gold"
        >
          {t('welcome_cta')}
          <ArrowRight className="w-5 h-5" />
        </button>

        <p className="text-xs text-center text-white/30">{t('welcome_trusted')}</p>
      </div>
        ...
        <div className="card-glass p-4 grid grid-cols-3 divide-x divide-white/5">
          {STATS.map(({ valKey, labelKey }) => (
            <div key={valKey} className="text-center px-3">
              <div className="font-display font-bold text-xl text-gradient-gold mb-0.5">
                {t(valKey)}
              </div>
        ...
        {[
          ...
          <div key={step.n} className="flex items-center gap-4 mb-4">
            <div className="w-8 h-8 rounded-full bg-gold-500/15 border border-gold-500/30 flex items-center justify-center flex-shrink-0">
              <span className="font-display font-bold text-sm text-gold-400">{step.n}</span>
            </div>
