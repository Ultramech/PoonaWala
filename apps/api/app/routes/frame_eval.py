...
async def live_guidance_ws(websocket: WebSocket):
    ...
    Frontend sends low-res frames, Gemini speaks back (or sends text to speak).
    """
    from google import genai
    from app.data.gemini import GEMINI_API_KEY, GEMINI_MODEL
    
    await websocket.accept()
    
    if not GEMINI_API_KEY:
        await websocket.send_json({"error": "GEMINI_API_KEY_MISSING"})
    ...
    try:
        # Connect to Gemini Live API with audio output enabled
        async with client.aio.live.connect(
            ...
            config={
                ...
                "system_instruction": "You are a helpful gold appraisal assistant. Guide the user to take a clear top-down photo of their jewelry. If the lighting is bad, say 'More light'. If too far, say 'Move closer'. If blurry, say 'Hold steady'. A ₹10 coin in the frame helps but is not required. Say 'Perfect, take it now!' when the jewelry is clear and in focus. Use very short, punchy guidance (3-5 words)."
            }
        ) as session:
            ...
            async def receive_from_frontend():
                try:
                    ...
                    logger.error(f"Guidance Frontend receive error: {e}")

            async def send_to_frontend():
                try:
                    async for message in session.receive():
                        if message.text:
                            await websocket.send_json({"text": message.text})
                        if message.data: # Audio data
                            await websocket.send_bytes(message.data)
                except Exception as e:
                    logger.error(f"Guidance Gemini receive error: {e}")
        ...
        logger.error(f"Live Guidance error: {e}")
